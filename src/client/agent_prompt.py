...
def build_system_prompt(tool_names_str: str) -> str:
    return (
        ...
        "CRITICAL TOOL RULES:\n"
         ...
         "2. SEARCH QUERIES: For hybrid_search_papers, pass the user's question in their own words — "
           "only resolve references ('that paper' -> the actual title) and drop conversational filler "
           "('can you find me', 'I want to know'). Keep words that carry the intent, like 'compares', "
           "'versus', 'fails at', 'improves'. For search_papers (arXiv keyword search), use a short "
           "keyword phrase instead: e.g. if the user says 'POPE stands for Privileged On-Policy "
           "Exploration', search for 'Privileged On-Policy Exploration'.\n"
